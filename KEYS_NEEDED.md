...
| Anthropic (portfolio agent interpreter) | API key (or `ant auth login` profile) | Required for a live agent demo | n/a | `claude-opus-5` structured outputs: natural-language goal -> GoalSpec. Never used for arithmetic, addresses or prices | https://console.anthropic.com/settings/keys | `ANTHROPIC_API_KEY` | Yes, usage-based (a few cents per goal) | No | Yes: the deterministic resolver is complete and tested | Server-side only. |
