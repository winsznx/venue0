...
| `VERIFIER_RPC_URL` | `wrangler.jsonc` vars, move to a secret when it carries a key | server | yes for independence | settlement verifier RPC; its host is recorded with each verification and compared with the executor host | Robinhood public RPC today: it returns HTTP 429 to Cloudflare Worker egress, so production verification falls back to the executor RPC and is recorded as not independent | no | configured, NOT WORKING from Workers; needs a second keyed provider (QuickNode, Chainstack or dRPC) with archive state |
...
- Schema: `apps/web/lib/server/db/schema.ts`, migrations `001_product`, `002_residual_quotes`, `003_lock_public_api`.
