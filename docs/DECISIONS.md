...
## D-020 Production deployment (2026-09-19)

- Host: Cloudflare Workers (owner's paid Workers plan) via `@opennextjs/cloudflare` 1.20.6 and wrangler 4.135.0, Worker `venue0` at `https://venue0.timjosh507.workers.dev`. Railway (trial expired) and Fly were evaluated and not used; an empty Fly app record created during evaluation was deleted.
- Database: Supabase Postgres, session pooler (IPv4, prepared statements supported; the transaction pooler would break them). The Worker reaches it through Cloudflare Hyperdrive (included in Workers Paid). Workers may not reuse sockets across requests, so the app opens one postgres.js client per request, keyed by the request's ExecutionContext (`fetch_types: false`, `max: 5`); Node keeps one process-wide client.
- Migrations run as a deploy step (`pnpm db:migrate`), in one transaction under a Postgres advisory lock, not at request time on Workers. `003_lock_public_api` enables row-level security with no policies on every table so Supabase's REST roles see nothing; the app connects as the owning role.
- Builds run from a clean copy of the repo (tracked and non-ignored files only) because OpenNext inlines any `.env` it finds into the Worker; `scripts/deploy/secret-scan.ts` blocks the deploy if any local `.env` value is in the output. The app no longer reads `.env` itself; local scripts pass it with `node --env-file`.
- Uniswap quotes between swap steps are stored in Postgres (`residual_quotes`, 60 s TTL), not process memory, since consecutive requests may hit different Worker instances.
- Verification records the executor and verifier RPC hosts and marks the run not independent when they match (executor Alchemy, verifier Robinhood public RPC in production).
- Observability: one JSON log line per API request with a request id (returned as `x-request-id`), user address, status and duration; round transitions, settlement reports and verifier results with tx hash and providers; step timings. Workers observability is enabled. Keys, tokens, cookies and connection strings are never logged.
- Performance: every held token's Chainlink feed is read at one pinned block in one multicall (`readChainlinkSnapshots`), with the same validation as the single reader. Production target check went from about 3.5 s to about 1.5 s.
- Dynamic: the same Sandbox environment as local testing, Robinhood Chain 4663 as the only network, Test Accounts enabled for operator-controlled e2e accounts. Login configuration was not weakened for testing. Allowed CORS origins list is empty (all origins allowed) per the owner's dashboard; restricting it to the Worker origin is recommended.
- Language agent: Groq (`openai/gpt-oss-120b`, strict JSON schema) when `GROQ_API_KEY` is set, else Anthropic, else weights mode only. Output is validated with the same Zod schema and still goes through the deterministic resolver.
- Addendum (2026-09-19, production testing): Hyperdrive now targets Supabase's direct connection, not the session pooler; each request's client holds one connection; a statement that failed to write (`write CONNECTION_CLOSED`, never sent) is retried once, nothing else is retried. Settlement reporting no longer waits inline: the report and each later poll do one short step (receipt check, then verification), because the edge closes idle connections at about 100 s and a request that waited for the receipt and verified inline was cancelled mid-way. Rounds left in SETTLING, SETTLED or VERIFYING resume on the next read. If the independent verifier RPC fails, verification reruns on the executor RPC and the record carries `independent: false` with the reason; the Robinhood public RPC rate-limits Cloudflare egress (HTTP 429 on the second call), so production needs a separately keyed verifier provider.
- Addendum (2026-09-19, independent verification): `VERIFIER_RPC_URL` is a Worker secret pointing at Chainstack; execution stays on Alchemy. Two keys on the same provider would not count as independent. JSON parameters now bind as `$n::text::jsonb`; with postgres.js a string bound as `$n::jsonb` is JSON-encoded again and stored as a JSON string. Migration `004` converted existing rows in place without changing any value.
