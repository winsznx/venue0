# Gate log (chronological)

The log as written during the build, oldest first. Early entries describe states that later gates superseded; the current summary is [../GATES.md](../GATES.md).
...
Blocked on: funded live wallets and gas (funding plan, since removed). No live gate is marked PASS.
...
Round P3 through the public product (not `/demo`): operator-controlled accounts A, B, C; A sells NVDA for SPY, B sells AAPL for NVDA, C sells SPY for AAPL. Evidence: `evidence/production/2026-09-19/p3-verification.json`, `prod-round3.log`.
