...
|---|---|---|---|---|---|---|
| C1 | "VENUE0 completed a 3-wallet Stock Token cycle atomically on Robinhood Chain." | PARTIAL | FORK | 1 round | `evidence/rehearsal/fork/L2-cycle/` | Fork rehearsal only. Becomes PROVEN only with a mainnet tx, explorer link and verifier PASS in `evidence/live/L2-cycle/`. |
...
| C4 | "Partial overlap crosses what it can and leaves an exact residual; nothing disappears." | PARTIAL | OFFLINE, FORK | property tests + 1 fork round | matcher property tests; `evidence/rehearsal/fork/L3-partial/` | Exactness is in raw units; USD residual may differ from requested minus crossed by at most 1 wei-USD per fill. |
| C5 | "With zero overlap VENUE0 returns NO_CROSS and does not manufacture a match." | PARTIAL | OFFLINE, FORK | unit tests + 1 fork round | `evidence/rehearsal/fork/L4-no-cross/` | No live run. |
