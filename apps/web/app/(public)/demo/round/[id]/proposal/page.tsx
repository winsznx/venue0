    ...
    <>
      ...
      <div className={styles.switcher} role="group" aria-label="View proposal as">
        ...
        {round.participants.map((p) => (
          <Link key={p.address} href={`/demo/round/${round.key}/proposal?as=${p.address}${demo ? "&demo=1" : ""}`} className={`chip ${p.address === me.address ? styles.chipActive : ""}`} aria-current={p.address === me.address ? "true" : undefined}>{p.label}</Link>
        ))}
      </div>
      ...
      <div className={styles.twoCol}>
        ...
        <section className={styles.sheet} aria-labelledby="constraints-title">
          ...
          <Link href={`/demo/round/${round.key}/execute${demo ? "?demo=1" : ""}`} className="btn btn-secondary" style={{ marginTop: 12 }}>See how it executed</Link>
        </section>
      </div>
    </>
  );
