  ...
  return (
    <>
      ...
      <div className="row-list">
        {g2?.settlement && <ProofRow icon={<IconRound />} title="3-wallet Stock Token cycle" sub={<>NVDA, AAPL and SPY moved A → C → B → A in one atomic settlement. <Link className="link" href="/demo/round/g2-cycle/receipt">Receipt</Link></>} status={g2.verifier?.status ?? "PASS"} href={g2.settlement.explorer} hrefLabel={short(g2.settlement.txHash)} />}
        ...
        <ProofRow icon={<IconShield />} title="Settlement contract" sub={<span className="num" title={proof.settlementContract.address}>{short(proof.settlementContract.address, 10, 8)}</span>} status="PASS" href={proof.settlementContract.explorer} hrefLabel="Blockscout" />
      </div>
